...
The model was trained on a large-scale dataset of 1.3M+ records which was derived from the CERT Insider Threat Dataset, achieving a F1-Score	0.9905 by successfully addressing extreme class imbalance and temporal dependency challenges.
...
```bash
git clone https://github.com/shikharjaveri/Temporal-Behavior-Modeling-for-Sequential-Anomaly-Detection.git && cd Temporal-Behavior-Modeling-for-Sequential-Anomaly-Detection
```
...
```
### Training the Model
To train the model using the optimized Large-LSTM parameters:

```bash
python train.py --epochs 10 --batch_size 2048 --seq_len 7
```
4. Running Evaluation
To evaluate the trained model and view classification metrics:
```bash
python eval.py
```
##  Performance Metrics
